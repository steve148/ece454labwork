...
#define MAX_DIM 16640 /* 32768 + 256 */
...
static int test_dim_rotate[] = {64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384};


/* Baseline CPEs (see config.h) */
static double rotate_baseline_cpes[] = {R64, R128, R256, R512, R1024, R2048, R4096, R8192, R16384};
...
int main(int argc, char *argv[])
{
	 ...
	 * the rotate() and bench() functions, but take average of 10 tries
	 */
	if (autograder) {
		rotate_benchmark_count = 10;

		int i;
        	for (i = 0; i < rotate_benchmark_count; i++) {
            		benchmarks_rotate[i].tfunct = rotate;
            		benchmarks_rotate[i].description = "rotate() function";
	...
	set_fcyc_cache_size(1 << 22); /* 4MB cache size */
	...
	if (autograder) {
		printf("\naverage score:%.1f\n", rotate_total_mean / rotate_benchmark_count);
	}
	else {
